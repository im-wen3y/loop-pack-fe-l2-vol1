# ProductListPage — 레이어 분리 근거

> 상세 표·고민 과정은 [`docs/week-03/separation-checklist.md`](../../docs/week-03/separation-checklist.md) 참고. 이 문서는 PR에서 바로 참조할 수 있는 핵심 요약.

## 레이어 구조

```
_apis/product/       API 호출 (endpoint, DTO) — TanStack Query를 모른다
_service/product/    캐싱 전략 (queryOptions, useGetProductList) — fetch를 모른다
_hooks/productList/  화면 전용 Custom Hook (상태·조합)
_utils/               순수 함수 (포맷, 계산)
_components/productList/  UI 렌더링 전용
productList/ProductListPage.tsx  위 조각을 조립하는 얇은 페이지
```

## Custom Hook — 한 문장 설명

| Hook                            | 설명                                                                             |
| ------------------------------- | -------------------------------------------------------------------------------- |
| `useProductFilters`             | 필터·검색·정렬 상태와 의미 기반 핸들러를 소유한다                                |
| `usePagination`                 | 현재 페이지 번호와 이동만 책임진다 (`totalPages`는 갖지 않음 — 쿼리 훅이 계산)   |
| `useProductListQuery`           | 필터·페이지를 서버 요청 DTO로 조립해 조회하고 화면용 형태로 반환한다             |
| `useSyncFiltersToUrl`           | 필터·페이지 상태를 URL 쿼리에 반영하고, 뒤로가기 시 URL에서 읽어 상태를 복원한다 |
| `useWishlist`                   | 위시리스트 id 목록을 유지하고 토글을 노출한다                                    |
| `useRecentlyViewed`             | 최근 본 상품 id를 최신순·중복 제거·최대 10개로 유지한다                          |
| `useLocalStorageState` (제네릭) | key·초기값을 받아 localStorage에 동기화되는 상태를 돌려준다                      |
| `useScrollToTop` (제네릭)       | dependency가 바뀌면 화면을 맨 위로 스크롤한다                                    |
| `useDebouncedValue` (제네릭)    | 값이 일정 시간 더 이상 바뀌지 않을 때만 최신값을 반영한다                        |

## 분리 근거 요약

- **`_api` vs `_service`** — `_apis`는 엔드포인트 호출만 알고 TanStack Query를 모른다. `_service`는 그 위에서 캐싱 전략만 다루고 `fetch`를 직접 호출하지 않는다. 페이지는 `_service`의 훅 하나만 알면 된다 (DIP).
- **`useProductListQuery`가 파라미터 조립까지 갖는 이유** — `_service`는 캐싱만 책임지고, 파라미터 조립·`totalPages` 파생 계산은 화면 관심사라 훅을 한 겹 더 뒀다.
- **`usePagination`과 `useProductFilters`를 합치지 않은 이유** — 조건(필터)과 위치(페이지)는 책임이 다르다. `onFilterChange` 콜백으로만 연결해 페이지 소유권을 `usePagination`에 유지했다.

## 분리하지 않은 것

| 대상                                         | 이유                                                                                                                                                                                                                                                    |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `viewMode` 상태                              | 부수효과·파생 없는 순수 UI 상태 1개. 훅으로 빼면 파일만 늘고 얻는 게 없다                                                                                                                                                                               |
| `loading`/`background-loading` 텍스트        | 로직 없는 1줄 텍스트라 분리해도 재사용·테스트 이득이 없다                                                                                                                                                                                               |
| `useSyncFiltersToUrl` 내부 URL 파라미터 조립 | 훅이 소유한 책임 그 자체. 밖으로 빼면 훅↔util 왕복만 늘어난다                                                                                                                                                                                           |
| 필터를 URL 상태로 완전히 이전하지 않은 것    | `useState` 소유 + `useSyncFiltersToUrl`로 URL 반영, 마운트 시 URL 읽어 초기값 복원까지 함. URL을 유일한 출처로 승격하지 않은 이유는 상태 관리 주체(useState)를 유지한 채로도 라우터 없이 왕복이 가능해서 — 별도 URL 상태 레이어를 만들 필요가 아직 없음 |

## 숨은 버그

| 증상                                                                   | 원인                                                                                                                                                             | 수정                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 새로고침·북마크 시 필터 초기화                                         | 필터를 URL에 반영만 하고 초기 마운트 시 읽어오지 않음                                                                                                            | `useProductFilters`/`usePagination`의 `useState`를 lazy initializer로 바꿔 마운트 시 URL을 1회 읽어 복원(`useLocalStorageState`와 동일 패턴). 라우터 없이 `URLSearchParams`만으로 충분 — 처음엔 "라우터 도입 시점으로 미룸"이라 적었지만 근거가 과장된 판단이었음                                                                                                                                               |
| API 오류 후 페이지 전체 새로고침 없이는 재시도 불가                    | `onRetry`가 `window.location.reload()`를 호출해 TanStack Query 캐시까지 날림                                                                                     | `useProductListQuery`가 반환하는 `refetch()`로 교체 — 캐시 유지한 채 해당 쿼리만 재조회                                                                                                                                                                                                                                                                                                                         |
| 검색어 입력마다 API 요청 발생                                          | `searchQuery` 변경이 debounce 없이 즉시 쿼리 파라미터에 반영됨                                                                                                   | `useProductListQuery`에서 `useDebouncedValue(filters.searchQuery, 300)`로 검색 조회만 debounce. 입력창은 debounce 없는 값을 그대로 써서 타이핑 반응성은 유지 (Playwright로 확인: 타이핑 중 0건, 입력 멈춘 뒤 300ms 후 1건)                                                                                                                                                                                      |
| 뒤로가기 시 이전 필터·페이지로 못 돌아감                               | `useSyncFiltersToUrl`이 매번 `replaceState`만 호출해 history entry가 쌓이지 않았고, `popstate`를 구독하지 않아 URL이 바뀌어도 상태가 갱신되지 않음               | 변화가 시작되는 시점의 "변경 전" URL을 `pushState`로 한 번 남겨두고, 그 뒤 값은 debounce(300ms)로 묶어 같은 entry를 `replaceState`로 갱신 — 타이핑 중 매 keystroke마다 entry가 쌓이지 않으면서도 뒤로가기 시 이전 상태로 복귀. `popstate` 시 URL을 읽어 `useProductFilters`/`usePagination` 상태를 되돌림 (Playwright로 확인: 카테고리→정렬 순으로 변경 후 뒤로가기 두 번이 각각 정렬→카테고리 순으로 되돌아감) |
| `?page=9999`처럼 실제 `totalPages`를 초과하는 값을 넣으면 빈 화면 노출 | `usePagination.ts`의 `readPageFromUrl`이 URL의 `page` 값을 상한 검증 없이 그대로 사용. `totalPages`는 쿼리 결과 이후에나 알 수 있어 훅 초기화 시점엔 클램프 불가 | `ProductListPage`에서 `totalCount` 기반 `totalPages` 응답을 받은 뒤(`!isLoading`) `page > totalPages`면 `goToPage(totalPages)`로 재보정. 두 훅의 결과를 모두 아는 조립 지점(페이지)에서 사후 보정 — 순환 의존(쿼리가 page를 요구하고, page 클램프는 쿼리 결과를 요구) 때문에 `usePagination` 내부에서는 처리 불가 (Playwright로 확인: `?page=99` 진입 시 `?page=3`으로 보정되고 실제 상품 목록 표시)            |

## 추가로 발견한 개선 여지 (미수정)

> `/self-review`로 위 3개 버그 수정을 검증하는 과정에서 추가로 발견한 nit 1건. 과제가 요구한 3개 버그와 달리 URL을 수동으로 편집해야만 발생하는 엣지케이스라 이번 범위에서는 수정하지 않음.

| 증상                                                                                        | 원인                                                                               | 왜 안 고쳤나                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `?minPrice=`처럼 URL에 빈 값으로 들어오면 `minPrice`가 `''`(미입력)이 아니라 `0`으로 복원됨 | `useProductFilters.ts`의 `parsePriceParam`이 `Number('')`(=0)을 유효한 값으로 판단 | 앱 자체의 URL 쓰기 경로(`useSyncFiltersToUrl`)는 이런 값을 만들지 않음 — URL을 직접 편집해야만 재현되는 케이스라 실사용 영향 없음. 고치려면 `value === ''` 케이스만 한 줄 추가하면 됨 |

...
| `entities/product`         | Product·ProductSort·Category 타입, 고정 pageSize, productQueries, useProductListQuery, 조회 parser와 정렬 허용값 | getProductList의 fetch 구현, serializeProductListQuery, 카테고리 parser 내부 허용값 | 도메인 타입과 목록 조회 계약 중 외부 소비처가 있는 심볼만 공개한다 |

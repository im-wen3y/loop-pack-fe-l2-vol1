...
| ~~미사용 자산(`components/ui/select`·`dialog` 등) 삭제~~ | ~~삭제는 구조 이동과 다른 종류의 변경이다. 이번엔 **배치만** 정하고 삭제 여부는 근거와 함께 따로 남긴다.~~ → **2단계에서 삭제로 결정을 바꿨다.** 이후 복원한 Select는 `shared/ui/select`의 controlled 상호작용 기반과 `entities/product`의 상품 옵션 정책·UI로 다시 분리했다. 범용 기반은 상품 필드를 모르고, 상품 옵션 UI는 아직 소비처가 없어 Public API로 열지 않는다. 재추가 근거로 들었던 "`ProductFilters`의 카테고리·정렬 select 대체"는 철회했다 — `TextOptionSelect`가 가격·재고를 아는 상품 옵션 UI라 `{id, label}`만 필요한 필터에는 애초에 안 맞았다. `ProductFilters`는 네이티브 `<select>`를 그대로 쓰고, 상품 옵션 select 3종은 Dialog처럼 소비처·재사용 계획이 없는 상태로 남는다. 상세 흐름은 [decisions.md](../week-06/decisions.md) 4번 |
...
└── shared/
    ...
    ├── lib/
    ├── styles/
    └── ui/{PageContainer,Pagination,select,Dialog}/   # select에는 controlled 상호작용 기반만 둔다
...
| `useSelect`, `SelectToggleIcon`                         | `shared/ui/select`                              | `shared` / — / `ui`            | controlled value와 키보드·바깥 클릭만 다루는 도메인 비종속 Select 기반 |
| 상품 옵션 select 3종·옵션 타입·품절 판단                | `entities/product/{model,ui}`                   | `entities` / `product`         | 가격·재고·할인·배송은 상품 도메인 정책과 표현                          |
| `dialog`                                                | `shared/ui/Dialog`                              | `shared` / — / `ui`            | controlled/uncontrolled 범용 UI이며 Select와 별도로 판단               |
...
| `entities/product`         | Product·ProductSort·Category 타입, 고정 pageSize, productQueries, useProductListQuery, 조회 parser와 허용값 | getProductList의 fetch 구현, serializeProductListQuery, 소비처 없는 상품 옵션 UI | 도메인 타입과 목록 조회 계약만 공개하고 상품 옵션 UI는 실제 조합 위치가 생길 때까지 숨긴다 |
